...
var S = {
  init: function () {
    ...
    if (i !== -1) {
      ...
      S.UI.simulate("|#countdown 3||You|Are|My|Someone|Special||#rectangle|");
    }
...
S.UI = (function () {
  ...
  // make sure we only show the alert once per load
  ...
  // extended helper that optionally calls `cb` when the timed sequence completes
    ...
    if (!max || (!reverse && currentAction < max) || (reverse && currentAction > 0)) {
      interval = setInterval(function () {
        ...
        if ((!reverse && max && currentAction === max) || (reverse && currentAction === 0)) {
          clearInterval(interval);
          interval = null; // mark as finished
  ...
  function performAction(value) {
    ...
    timedAction(function (index) {
      ...
      switch (action) {
        case 'countdown':
          ...
          }, 1000, value, true, function () {
            // after countdown finishes, check if there are remaining items
            if (sequence.length === 0) {
              console.log('countdown complete, will popup in 1s');
    ...
    }, 2000, sequence.length, false, function () {
  ...
  // show the feedback alert using SweetAlert
  function showPopup() {
    console.log('>>> showPopup called, popupShown=', popupShown, 'Swal=', typeof Swal);
    if (popupShown) return;
    popupShown = true;

    if (typeof Swal === 'undefined') {
      var text = prompt('apa perasaan kamu saat lihat ini');
      if (text) {
        var msg = encodeURIComponent(text);
        var url = 'https://api.whatsapp.com/send?phone=6282123432959&text=' + msg;
    ...
    Swal.fire({
      ...
      didOpen: function () {
        // ensure alert has highest z-index
        var c = Swal.getContainer();
  ...
  function bindEvents() {
    // no input field present; keyboard controls are disabled
    // keeping body keydown listener empty if needed later
  ...
  function init() {
    ...
    // expose popup helper so callbacks can use it
